...
const AdminDataCollection = () => {
    ...
    const renderDataCard = (item, type) => {
        if (type === 'supply') {
            ...
            return (
                <div key={item.id} className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-xl p-4 flex flex-col gap-3 shadow-lg">
                    <div className="flex justify-between items-center border-b border-[var(--theme-border)] transition-colors duration-500 pb-2"><span className="text-white font-black text-sm">{item.stock_name || '종목'}</span><span className="text-[10px] text-slate-500 font-mono">{formatTimeCompact(item.captured_at)}</span></div>
        ...
        if (type === 'rank') {
            return (
                <div key={item.id} className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-xl p-4 flex flex-col gap-1"><div className="flex justify-between items-center mb-1"><span className="px-1.5 py-0.5 bg-cyan-500/10 text-cyan-400 text-[9px] font-black rounded border border-cyan-500/20">{item.ranking_type}</span><span className="text-[9px] text-slate-500">{formatTimeCompact(item.captured_at)}</span></div><div className="flex justify-between items-center"><span className="text-white font-bold text-xs">{item.stock_name || item.stock_code}</span><span className="text-indigo-400 font-black font-mono text-xs">{item.rank_value}</span></div></div>
            );
        }
        return (
            <div key={item.id || Math.random()} className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-xl p-4"><div className="flex justify-between items-center"><span className="text-white font-bold text-xs truncate max-w-[60%]">{item.theme_name || item.industry_name}</span><span className={classNames("text-[11px] font-black font-mono", parseFloat(item.change_rate) >= 0 ? "text-rose-400" : "text-blue-400")}>{item.change_rate}%</span></div><span className="text-[9px] text-slate-500 block text-right mt-1">{formatTimeCompact(item.captured_at)}</span></div>
        );
    };
    ...
    return (
        <div className="flex-1 bg-[var(--theme-bg)] transition-colors duration-500 p-4 lg:p-6 overflow-y-auto custom-scrollbar relative h-full flex flex-col font-sans">
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                ...
                <div className="flex bg-[var(--theme-header)] transition-colors duration-500/80 p-1 rounded-xl border border-[var(--theme-border)] transition-colors duration-500 self-stretch lg:self-auto shadow-lg overflow-x-auto no-scrollbar">
                    {[
            ...
            {activeTab === 'overview' && (
                <div className="grid grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="col-span-12 lg:col-span-8 bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-2xl p-6 shadow-xl">
                        <div className="flex justify-between items-center mb-6"><h2 className="text-lg font-bold text-white flex items-center gap-2"><BarChart3 size={20} className="text-indigo-400" /> 수집 현황</h2><div className="text-[10px] font-black bg-slate-800 text-slate-400 px-3 py-1.5 rounded-lg border border-[var(--theme-border)] transition-colors duration-500 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> ACTIVE</div></div>
                    ...
                    <div className="col-span-12 lg:col-span-4 space-y-6">
                        <div className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-2xl p-6 shadow-xl">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[{v: 30, l: '30초'}, {v: 60, l: '1분'}, {v: 180, l: '3분'}, {v: 300, l: '5분'}, {v: 420, l: '7분'}, {v: 600, l: '10분'}].map(item => (
                                    <button key={item.v} onClick={() => handleIntervalChange(item.v)} className={classNames("py-3 rounded-xl text-[10px] font-black border transition-all", config.collect_interval === item.v ? "bg-indigo-600/20 border-indigo-500 text-indigo-400" : "bg-slate-800/50 border-[var(--theme-border)] transition-colors duration-500 text-slate-500")}>{item.l}</button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-2xl p-6 shadow-xl">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center bg-slate-800/30 p-3 rounded-xl border border-[var(--theme-border)] transition-colors duration-500">
                                    <span className="text-xs font-black text-slate-400">주말 수집 허용</span>
                                    <div className="flex bg-[var(--theme-bg)] transition-colors duration-500 p-1 rounded-lg border border-[var(--theme-border)] transition-colors duration-500">
                        ...
                        <div className="bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-2xl p-6 shadow-xl flex flex-col items-center justify-center text-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                            <div className={classNames("p-5 rounded-2xl border transition-all duration-500", aiMode === 'STABLE' ? "bg-indigo-600/10 border-indigo-500/50 shadow-xl" : "bg-[var(--theme-bg)] transition-colors duration-500/50 border-[var(--theme-border)] transition-colors duration-500 opacity-40 grayscale")}>
                                ...
                                <ul className="space-y-2.5 text-[11px] font-bold text-slate-400">
                                    ...
                                    <li className="pt-2 border-t border-[var(--theme-border)] transition-colors duration-500/50 text-[10px] text-slate-500 italic leading-relaxed">"확실한 패턴이 완성된 우량주 위주의 선별"</li>
                                </ul>
                            </div>
                            <div className={classNames("p-5 rounded-2xl border transition-all duration-500", aiMode === 'BALANCED' ? "bg-emerald-600/10 border-emerald-500/50 shadow-xl" : "bg-[var(--theme-bg)] transition-colors duration-500/50 border-[var(--theme-border)] transition-colors duration-500 opacity-40 grayscale")}>
                                ...
                                <ul className="space-y-2.5 text-[11px] font-bold text-slate-400">
                                    ...
                                    <li className="pt-2 border-t border-[var(--theme-border)] transition-colors duration-500/50 text-[10px] text-slate-500 italic leading-relaxed">"알고리즘과 AI의 조화로운 중단기 분석"</li>
                                </ul>
                            </div>
                            <div className={classNames("p-5 rounded-2xl border transition-all duration-500", aiMode === 'NEUTRAL' ? "bg-cyan-600/10 border-cyan-500/50 shadow-xl" : "bg-[var(--theme-bg)] transition-colors duration-500/50 border-[var(--theme-border)] transition-colors duration-500 opacity-40 grayscale")}>
                                ...
                                <ul className="space-y-2.5 text-[11px] font-bold text-slate-400">
                                    ...
                                    <li className="pt-2 border-t border-[var(--theme-border)] transition-colors duration-500/50 text-[10px] text-slate-500 italic leading-relaxed">"데이터의 완벽한 수평 균형"</li>
                                </ul>
                            </div>
                            <div className={classNames("p-5 rounded-2xl border transition-all duration-500", aiMode === 'AGGRESSIVE' ? "bg-rose-600/10 border-rose-500/50 shadow-xl" : "bg-[var(--theme-bg)] transition-colors duration-500/50 border-[var(--theme-border)] transition-colors duration-500 opacity-40 grayscale")}>
                                ...
                                <ul className="space-y-2.5 text-[11px] font-bold text-slate-400">
                                    ...
                                    <li className="pt-2 border-t border-[var(--theme-border)] transition-colors duration-500/50 text-[10px] text-slate-500 italic leading-relaxed">"미세한 수급 변화까지 감지하여 기회 극대화"</li>
                                </ul>
                            </div>
                        </div>
                        <div className="mt-10 p-6 bg-[var(--theme-bg)] transition-colors duration-500 rounded-2xl border border-[var(--theme-border)] transition-colors duration-500 flex items-start gap-4"><AlertCircle className="text-amber-500 shrink-0 mt-1" size={20} /><div><h4 className="text-amber-500 font-black text-xs uppercase tracking-widest mb-1">Notice</h4><p className="text-slate-400 text-[11px] leading-relaxed">전략 변경 시 다음 분석 사이클부터 즉시 반영됩니다.</p></div></div>
                    </div>
                </div>
            )}
            ...
            {selectedLog && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-[var(--theme-header)] transition-colors duration-500 border border-[var(--theme-border)] transition-colors duration-500 rounded-2xl shadow-2xl overflow-hidden flex flex-col"><div className="p-4 border-b border-[var(--theme-border)] transition-colors duration-500 bg-[var(--theme-header)] transition-colors duration-500 flex justify-between items-center text-white"><h3 className="font-bold text-sm">Log Detail</h3><button onClick={() => setSelectedLog(null)}><X size={18}/></button></div><div className="p-6 bg-[var(--theme-bg)] transition-colors duration-500 font-mono text-[11px] text-slate-200 overflow-y-auto max-h-[50vh] leading-relaxed whitespace-pre-wrap">{selectedLog.message || `No message content.`}</div></div>
                </div>
            )}
        </div>
    );
