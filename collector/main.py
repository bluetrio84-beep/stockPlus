...
# --- 3. 메인 루프 ---
def main():
    mega = MegaCollector(); trader = DaumTraderScraper()
    while True:
        try:
            ...
            if 8 <= now_hour < 23:
                ...
                mega.run_deep_analysis(sects, themes)
                
                time.sleep(interval)
            else:
                mega.update_stats(1); time.sleep(600)
        except Exception as e: 
            try: mega.log_to_db("ERROR", f"[MainLoop] {str(e)}")
            except: pass
