...
class BlackBoxAnalyst:
    ...
    def calculate_tactical_tags(self, code, curr_price, curr_vol, f_buy):
        ...
        try:
            with self.conn.cursor(pymysql.cursors.DictCursor) as cursor:
                # [v49.0] 수급 및 기술 분석 데이터 조회 (OBV 포함)
                cursor.execute("SELECT price, volume, rsi, ma5, ma20, program_net_buy, obv FROM stock_intraday_history WHERE stock_code = %s ORDER BY id DESC LIMIT 5", (code,))
