...
### 2. 가점 이중 반영 해소 및 레이어 분리 (`collector/next_leader_engine.py`)
- **문제점 진단**:
  - 프로그램 수급 가점(`p_boost`, 최대 15.5점)과 공매도 숏커버 가점(`s_boost`, 최대 20.5점)이 퀀트 점수(Q)와 AI 3대 모델 점수(L, T, X) 양쪽에 중복 합산되던 구조적 결함.
  - 이로 인해 점수 인플레이션이 발생하고 DB에 기록되는 AI 모델 점수가 훼손되어 주말 최적화 가중치 평가 시 왜곡 초래.
- **개선 및 반영 내역**:
  - **퀀트(Q)**: 기술적 지표 + 수급(`p_boost`) + 공매도(`s_boost`) 가점을 집약 반영.
  - **AI 모델(L, T, X)**: 순수 AI 시계열 예측 점수를 온전히 보존하고 기업 실적 가점(`f_boost`)만 연계하도록 분리.
### 3. AI 자가진화(Self-Evolution) 재학습 파이프라인 가동 및 스케줄러 연동 (`collector/main.py`)
- **문제점 진단**:
  - 기존 딥러닝 가중치 파일(`stock_lstm_v1.pth`, `stock_tcn_v1.pth`, `stock_xgb_v1.json`)이 2026년 3월 이후 정기 재학습 없이 고정되어 있었음.
...
- **프론트엔드(`AdminIntelligenceDashboard.jsx`) UI 고도화**:
  ...
  - **텍스트 시인성 극대화 (화이트 테마 검은색 폰트 & 다크모드 동시 지원)**:
    - `pure-white`(화이트 배경) 모드에서 글자가 묻히지 않도록 하드코딩된 흰색을 제거하고 테마 텍스트 변수(`text-[var(--theme-text)]`)를 전면 적용.
    - 화이트 배경에서는 또렷한 **검은색/진남색(#0f172a)**, 다크모드에서는 밝은 **화이트/슬레이트(#e2e8f0)**로 자동 전환되어 6종 전 테마에서 완벽한 가독성 확보.
    - 5단계 국면별 뱃지 컬러(에메랄드, 블루, 인디고, 앰버, 로즈) 및 순매수 텍스트 강조.
  - **5단계 국면 가이드 툴팁/모달 신설 (`HelpCircle`)**:
    - `0 ~ 30점`: 🚨 바닥 매수 유효 (BOTTOM_BUY) - 투매 절정, 손절 자제 및 분할 매수 타점
    - `31 ~ 45점`: 🔍 수급 관망 눌림목 (CAUTION_ACCUMULATE)
    - `46 ~ 60점`: ⚖️ 중립 국면 (NEUTRAL) - 박스권 개별 섹터 순환매 장세
    - `61 ~ 75점`: 💰 수익 실현 조짐 (PROFIT_TAKING)
    - `76 ~ 100점`: ⚠️ 상투 과열 경보 (PEAK_WARN) - 과열 경보, 추격 매수 금지 및 현금 비중 확대
  - **핵심 UI 컴포넌트**:
    1. *상단 종합 알림 배너*: 공포/탐욕 수치(X/100), 국면 라벨, 실전 대응 가이드 및 국면 가이드 팝업 버튼.
