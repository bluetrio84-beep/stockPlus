            ...
            <div>
                <h2 className="text-base font-black text-[var(--theme-text)] flex items-center gap-2 transition-colors">
                    ...
                    {getStockStatusBadge(stock) && (
                        <span className={classNames("text-[10px] font-black px-1.5 rounded border leading-tight transition-colors", getStockStatusBadge(stock).color)}>
                    ...
                    <span className="text-xs font-bold text-slate-500 flex items-center gap-1.5 transition-colors">
                        ...
                        {stock.aiScore !== undefined && stock.aiScore !== null && (
                            <span className={classNames("text-[9px] font-black px-1.5 py-0.5 rounded-full border leading-tight flex items-center gap-1 transition-all transition-colors", 
                                ...
                                <span className={classNames("w-1 h-1 rounded-full transition-colors", 
        ...
        {activeTab === 'chart' && (
          <div className="flex flex-col h-full transition-colors">
            <div className="px-2 py-1.5 border-b border-[var(--theme-border)] bg-[var(--theme-header)] flex items-center gap-1 overflow-x-auto no-scrollbar shrink-0 transition-colors duration-500 shadow-sm">
              <button onClick={() => handleAiAnalysis && handleAiAnalysis()} className="flex items-center gap-1 text-[10px] font-black text-yellow-600 bg-yellow-500/10 px-2.5 py-1.5 rounded-lg shrink-0 transition-all active:scale-95 transition-colors"><Sparkles size={12} /> AI 분석</button>
              <div className="h-3 w-px bg-[var(--theme-border)] mx-0.5 transition-colors"></div>
              <button onClick={() => onExchangeChange && onExchangeChange()} className={classNames("flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-black rounded-lg border shrink-0 transition-all shadow-sm active:scale-95 transition-colors", marketInfo.colorClass.replace('600/10', '600/20'))}><Repeat size={10} /> {marketInfo.name}</button>
              <div className="flex gap-1 ml-auto border-l border-[var(--theme-border)] pl-2 transition-colors">
                  {['5m', '1D', '1W', '1M'].map(p => ( <button key={p} onClick={() => onPeriodChange && onPeriodChange(p)} className={classNames("px-2.5 py-1.5 text-[10px] font-black rounded-lg transition-all active:scale-95 transition-colors", currentPeriod === p ? "bg-[var(--theme-point)] text-white shadow-lg shadow-[var(--theme-point)]/20" : "bg-[var(--theme-bg)] text-slate-500 hover:text-[var(--theme-text)] border border-[var(--theme-border)]")}>{p}</button> ))}
              </div>
            </div>
            <div className="flex-1 w-full relative transition-colors">
                ...
                {!isDataLoaded && <div className="absolute inset-0 flex items-center justify-center bg-[var(--theme-header)]/50 z-10 backdrop-blur-sm transition-colors"><Loader2 className="animate-spin text-[var(--theme-point)]" size={32} /></div>}
            </div>
          </div>
        )}
        {activeTab === 'daily' && <div className="absolute inset-0 flex flex-col transition-colors"><DailyPriceTable prices={dailyPrices} /></div>}
        {activeTab === 'investors' && <div className="absolute inset-0 flex flex-col transition-colors"><InvestorTable data={investorsData} isDataLoaded={isDataLoaded} /></div>}
        {activeTab === 'traders' && <div className="absolute inset-0 flex flex-col transition-colors"><TraderTable traderData={traderData} /></div>}
      </div>
